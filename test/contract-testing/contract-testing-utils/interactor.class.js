...
async function commonMessageHandler(message, handler, assertFunc) {
...
class Interactor {
    constructor(options, messageHandler) {
        if (!messageHandler)
            throw new Error('Missing the following parameter: messageHandler');
    ...
    defineAsyncInteraction (state, eventName, content, assertFunc) {
        return (
            this.messagePact
                ...
                .verify(asynchronousBodyHandler(message => commonMessageHandler(message, this.messageHandler, assertFunc)))
        );
    }
